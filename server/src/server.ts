...
import routerProductCategory from "./routes/productCategory";
import routerAuth from "./routes/authRoute";
import db from "./connections/db-connection";
import { setupAssociations } from "./connections/associations";
...
class Server {
  ...
  middlewares() {
    ...
    const limiter = rateLimit({
      ...
      max: 200, // Limita a 100 solicitudes por IP
      message:
  ...
  routes() {
    ...
    this.app.use("v1/api/product/", routerProductCategory)
